// 2-LBM solver with drag-based force coupling
...
int algoLB (Vec2D &rho_m, Vec2D &ux_m, Vec2D &uy_m, Vec3D &f1_m, Vec3D &f2_m, 
    ...
    for (int i = 0; i < NX; i++) {
        for (int j = 0; j < NY; j++) {
            ...
            // Drag-based force coupling
            double reduced_mass = 2. * Rp * Rm / (Rp + Rm);
            double collision_freq = 0.1;
            Fx_m = -(Ex[i][j] + uy_m[i][j] * Bz[i][j]) + reduced_mass * collision_freq * (upx - umx);
            Fy_m = -(Ey[i][j] - ux_m[i][j] * Bz[i][j]) + reduced_mass * collision_freq * (upy - umy);

            Fx_p = (Ex[i][j] + uy_p[i][j] * Bz[i][j]) + reduced_mass * collision_freq * (umx - upx);
            Fy_p = (Ey[i][j] - ux_p[i][j] * Bz[i][j]) + reduced_mass * collision_freq * (umy - upy);
...
int main() {
    ...
    std::cout << "Relaxation time: " << tau << ", Spatial step: " << dx_phys << "m" << std::endl;
    ...
    // ------------------------ Time loop ------------------------
    const int Nsteps = std::ceil(10. / dt_phys), n_out = 20;
    int check = 0;
